...
A `kubectl`-only protocol measures the OVH MKS storage behaviours that **decide the config values
and validate the design assumptions**. The full protocol and raw `results.md` live in the local
`.plans/` (git-ignored); this is the durable summary and how each finding maps to config.
...
## Cluster facts that shape the configuration

- `csi-cinder-high-speed-gen2` uses **`volumeBindingMode: Immediate`**: PVCs bind without a
  consumer, so consumer pods exist only to trigger the attach/mount that is being timed.
- Both `csi-cinder-snapclass-v1` **and** `csi-cinder-snapclass-in-use-v1` already exist — **reuse
  them**, don't create a `cinder-snap`. The *in-use* variant is the key enabler for durability
  snapshots.
...
| **D — snapshot + clone** | **CoW**: time is ~constant regardless of data size | cloning a warm cache is cheap → horizontal fan-out is viable | `spec.fanout` uses CoW clones |
| **In-use snapshot probe** | **supported** (snapshot a still-mounted PVC) | snapshot a hot daemon **without** scaling it to zero | durability snapshots use `csi-cinder-snapclass-in-use-v1` |
...
- **D is the reason fan-out is a CoW clone, not a full copy.** Constant-time snapshot/clone
  means materializing N warm replicas from a snapshot is practical.
- **In-use snapshots simplify durability.** Because OVH can snapshot a still-mounted volume,
  durability snapshots run against a live, hot daemon — no scale-to-zero needed to snapshot.
