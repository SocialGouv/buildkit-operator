...
| [poc-pic-oidc.md](poc-pic-oidc.md) | Reproducible method for the PoC: build from the SocialGouv PIC (vendored `build.sh` + inline CI through the egress proxy) and the staged, zero-downtime rollout that moves `/route` auth from a shared bearer to per-build **OIDC** (code → GitOps → `helm upgrade` go-live → flip consumers → strict). |
