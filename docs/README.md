...
| [ci-integration.md](ci-integration.md) | The CI-agnostic integration (GitHub/GitLab/any runner), the public exposure (LB + mTLS + cert SANs), and the live example repo. |
...
- **Integration is CI-agnostic** — a single `build.sh` (route → `buildx remote` over mTLS) works on
  a stock GitHub-hosted runner and on GitLab unchanged.
